...
public class GestioneTab : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_check_per_CanvasPOI)
        {
            ...
            if (PlayerPrefs.GetInt("percorso_selezionato") > 0 && _btn_List[1].gameObject.activeSelf)
            {
                selectes_btn_index(0);
                _btn_List[1].gameObject.SetActive(false);
            }
            else if (!(PlayerPrefs.GetInt("percorso_selezionato") > 0) && !_btn_List[1].gameObject.activeSelf)
            {
                _btn_List[1].gameObject.SetActive(true);
                if (!_btn_List[1].enabled)
                    _btn_List[1].enabled = true;
            }
            if (PlayerPrefs.GetInt("percorso_selezionato") > 0 && !_btn_List[2].gameObject.activeSelf)
            {
                _btn_List[2].gameObject.SetActive(true);
                if (!_btn_List[2].enabled)
                    _btn_List[2].enabled = true;
            }
            else if (!(PlayerPrefs.GetInt("percorso_selezionato") > 0) && _btn_List[2].gameObject.activeSelf)
