...
public class ItinerariEventiPOI_AttaccatiAlComune : MonoBehaviour
{
    ...
    private bool _bici = false;
    private bool _piedi = false;
    ...
    {
        _DBClass = GameObject.FindWithTag("SQLite").GetComponent<DBClass>();
        int _poi_selezionato = PlayerPrefs.GetInt("poi_selezionato");
        ...
        {
            foreach (var _poiAssociatoApercorsi in _DBClass.GetPERCORSO(null, _poi_selezionato))
            {
            ...
            foreach (var _poixtappe in _DBClass.getPOIXTAPPE(null, _poi_selezionato))
            {
            ...
            foreach (var _percorso in _PERCORSOs)
            {
                if (_percorso != null)
                {
                    ...
                    if (_percorso.poi_id > 0)
                    {
                        ...
                        var _poi = _DBClass.getPOI(_percorso.poi_id).FirstOrDefault();
                        if (_poi != null)
                            _IEP.nome_comune = _poi.comune + $" ({_poi.provincia})";
                    }
