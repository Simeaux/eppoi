...
namespace ARLocation.MapboxRoutes
{
    public class MapboxRoute : MonoBehaviour
    {
        ...
        public class SettingsData
        {
            ...
            public MapboxApiLanguage Language = MapboxApiLanguage.Italian;
        ...
        void Awake()
        {
            ...
            mapbox = new MapboxApi(Settings.MapboxToken, lang);
        }
        ...
        void Update()
        {
            if (NumberOfSteps > 0)
            {
                ...
                {
                    if (s.SignPostInstances.Count > i)
                    {
        ...
        private SignPostEventArgs createSignPostEventArgs(int index)
        {
            ...
            //var target = GPSEncoder.GPSToUCS(new Vector2((float)t.Longitude, (float)t.Latitude));
            //var user = GPSEncoder.GPSToUCS(new Vector2(_long, _lati));
            var user = Camera.main.transform.position;
            var target = s.StepsPlaceAtInstances[index].transform.position;
            ...
            if (string.IsNullOrEmpty(s.RouteSteps[index].maneuver.instruction.Trim()))
            {
                ...
                if (index < s.RouteSteps.Count - 1)
                {
                    ...
                    for (int i = index; i < s.RouteSteps.Count - 1; i++)
                    {
                        ...
                        var b_loc = s.StepsPlaceAtInstances[i + 1].LocationOptions.LocationInput.Location;
                        ret += GetDistance(a_loc.Longitude, a_loc.Latitude, b_loc.Longitude, b_loc.Latitude);
                    }
                }
            }
            return ret;
        ...
        public void ClosestTarget()
        {
            ...
            if (currentIndex < 0)
            {
                Debug.Log("MapboxRoute - ClosestTargetByUserPosition - currentIndex < 0");
                Utils.Logger.WarnFromMethod("MapboxRoute", "ClosestTargetByUserPosition", "currentIndex < 0");
                return;
            }

            Debug.Log("MapboxRoute - ClosestTargetByUserPosition" + currentIndex);
